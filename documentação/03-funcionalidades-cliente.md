...
2. Removemos validações de campo = 0 dos métodos dos formulários, e colocamos apenas no cControlo (regra de negocio) - existia Redundância.
3. `using var` na Camada 3 (`cMovimento`, `cAdmin`): todos os `NpgsqlCommand` e `NpgsqlDataReader` usam `using var` para garantir que `Dispose()` é chamado automaticamente ao sair do scope — mesmo com exceção. Sem `using var`, o `Dispose()` só corria no caminho sem erros.
4. `fTraduzirTipo` usa **switch expression** (C# 8+) em vez de `switch` clássico - sintaxe mais compacta, resultado idêntico. `"D" => "Depósito"` equivale a `case "D": return "Depósito"`. `_` equivale a `default`.
